...
| B7 | **Minimal service worker for standalone PWA install** | P1-7 device test | On-device (Android Chrome) the manifest+icons install as a *browser shortcut*, not a fullscreen WebAPK — Chrome needs a registered service worker. Add a **network-first / no-cache passthrough** SW (just enough for installability; avoid stale-content caching). Slice of P2-3 pulled forward. Re-test standalone launch after. |
| B8 | **PWA install must be tested on a clean origin, not ngrok-free** | P1-7 device test | ngrok-free's interstitial breaks Google's WebAPK icon/start_url fetch. Test via Cloudflare quick tunnel or the deployed domain. Documented in `P1-7-pwa.md` + `TEST-PLAN.md`. |
